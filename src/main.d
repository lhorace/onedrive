...
import std.file, std.path, std.process, std.stdio, std.conv, std.algorithm.searching;
...
Options:
  ...
  --confdir ARG
       ...
       [default: " ~ configDirBase ~ "/onedrive" ~ "]
...
";
	...
	string configDirName = arguments["--confdir"].toString;
	// only download remote changes
	bool downloadOnly = arguments["--download-only"].toString == "true";
	// override the sync directory
	string syncDirName = arguments["--syncdir"].toString;
	// enable monitor mode
	bool monitor = arguments["--monitor"].toString == "true";
	// force a full resync
	bool resync = arguments["--resync"].toString == "true";
	// remove the current user and sync state
	bool logout = arguments["--logout"].toString == "true";
	...
	bool debugHttp = arguments["--debug-https"].toString == "true";
	// This allows for selective directory syncing instead of everything under ~/OneDrive/
	string singleDirectory = arguments["--single-directory"].toString;
	// Create a single root directory on OneDrive
	string createDirectory = arguments["--create-directory"].toString;
	// Remove a single directory on OneDrive
	string removeDirectory = arguments["--remove-directory"].toString;
	// The source directory if we are using the OneDrive client to rename a directory
	string sourceDirectory = arguments["--source-directory"].toString;
	// The destination directory if we are using the OneDrive client to rename a directory
	string destinationDirectory = arguments["--destination-directory"].toString;
	...
	bool synchronize = arguments["--synchronize"].toString == "true";
	// Local sync - Upload local changes first before downloading changes from OneDrive
	bool localFirst = arguments["--local-first"].toString == "true";
	// Upload Only
	...
	bool noRemoteDelete = arguments["--no-remote-delete"].toString == "true";
	// Do not use notifications in monitor mode
	bool disableNotifications = arguments["--disable-notifications"].toString == "true";
	// Display application configuration but do not sync
	bool displayConfiguration = arguments["--display-config"].toString == "true";
	...
	bool disableUploadValidation = arguments["--disable-upload-validation"].toString == "true";
	// Do we enable a log file
	bool enableLogFile = arguments["--enable-logging"].toString == "true";
	// SharePoint / Office 365 Shared Library name to query
	string o365SharedLibraryName = arguments["--get-O365-drive-id"].toString;
